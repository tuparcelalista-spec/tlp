...
function calculate(input){
  ...
  const routeKm=Math.max(0,Number(input.routeDistanceKm)||0);
  const valorFinal = technicalPotential;
  const valorComunal = input.promedio_m2_comunal ? roundPrice(input.promedio_m2_comunal * area) : (market?.medianValue||((market?.medianM2&&area)?roundPrice(market.medianM2*area):0));
  ...
  const valorTplTasadorAjustado=valorFinal;
  const valorTplTasador=valorFinal;
  ...
  const valorTplTasadorComuna=valorTplTasador;
  const valorVentaApuro=valorPorApuro;
  ...
  const immediateReference=normalize(input.tourism)==='nacional'?null:roundPrice((valorTplTasador*.90+immediateBase)/2);
  ...
  const asking=Number(input.asking)||0,publishedM2=asking&&area?Math.round(asking/area):null,tplM2=area?Math.round(valorTplTasador/area):null,marketM2=market?.medianM2||null;
  ...
  return {
    // — TPL Tasador V2: fuente única —
    valorFinal,
    valorComunal,
    valorPromedioReferencia,
    valorPorApuro,
    quick:agile, ideal:recommended, patient,
    agile, recommended, /* @deprecated → usar valorFinal */ technicalPotential:valorTplTasadorBase, patientPotential:patient, immediateReference,
    ...
    reference:valorTplTasador, low:agile, high:valorTplTasadorBase,
    ...
    marketReference:market,marketBlend:{technicalWeight:1,marketWeight:0,independent:true,isolationApplied:false}, /* @deprecated — solo compatibilidad, nunca modifica valorFinal */
    ...
    calibration:{profile:CALIBRATION_PROFILE.version,anchor:CALIBRATION_PROFILE.anchor,
      anchorAreaM2:CALIBRATION_PROFILE.anchorAreaM2,anchorValueCLP:CALIBRATION_PROFILE.anchorValueCLP,
      factor:1,appliesGlobally:false},
