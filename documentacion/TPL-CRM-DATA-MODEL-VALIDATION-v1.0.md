...
### 4. ¿Qué debe ampliarse?
* `tpl_actores` y `tpl_actor_roles` no requieren cambios de esquema. Deben ampliarse únicamente los valores semánticos almacenados dentro de la columna flexible `metadata` (ej. bitácora de llamadas, tags de UTM, etc.).
* `tpl_oportunidades` requiere la adición de una columna o valor de sub-estado comercial para mapear las fases del Kanban.
