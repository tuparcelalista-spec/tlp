  ...
  // El calculo lo hace siempre el motor unico (js/core/valuation-engine.js),
  // que la pagina ya carga. Aqui solo se arma su entrada desde la fila del
  // catalogo; no vive ninguna formula de valor en este archivo.
  async function calculateTplAsync(p){
    const area=areaOf(p), asking=priceOf(p), ctx=distanceContext(p);
    if(!area || !ctx) return null;
    ...
    // Esto llamaba a /api/tasador, que es un stub que responde
    // {valor_recomendado: 0, error: 'Use local engine'}. Con eso tplM2 salia 0
    // y TODA parcela del catalogo quedaba clasificada como "Sin lectura
    // suficiente": esta seccion llevaba tiempo sin decir nada.
    // El motor unico ya esta cargado en la pagina, y el payload de arriba es
    // exactamente su formato de entrada.
    const motor = typeof window !== 'undefined' ? window.TPLLandEngine : null;
    if (!motor?.calculate) return null;
    const resultado = motor.calculate(payload);
  ...
  function classify(p, tpl, market){
    ...
    const tplM2=tpl?.valorFinal&&area?Math.round(tpl.valorFinal/area):0;
    // medianM2Ajustado ya viene corregido a la superficie real de la propiedad
    // dentro de su tramo; medianM2 es la mediana cruda de la muestra.
    const marketM2=market?.medianM2Ajustado || market?.medianM2 || 0;
  ...
  function commercialTier(p, analysis){
    ...
    const price=Number(a?.price||0), tplValue=Number(a?.tpl?.valorFinal||0);
  ...
  // analyze() sincrónico ya no puede calcular el TPL en tiempo real.
  function analyze(p){
    return { area: areaOf(p), price: priceOf(p), error: 'Use analyzeAsync' };
  }

  async function analyzeAsync(p){
    const area=areaOf(p), price=priceOf(p), publishedM2=area&&price?Math.round(price/area):0;
    const tpl=await calculateTplAsync(p);
    // La referencia comunal sale del propio motor, que ya eligio la mediana del
    // tramo de superficie que corresponde a esta propiedad.
    const market=tpl?.marketReference||null;
    const tplM2=tpl?.valorFinal&&area?Math.round(tpl.valorFinal/area):0;
    const classification=classify(p,tpl,market);
    const technicalValue=Number(tpl?.valorTplTasadorAjustado||0);
    const observedCommunalValue=Number(tpl?.valor_comunal||0);
    // Antes esta linea rehacia por su cuenta el promedio tecnico/comunal. Esa
    // mezcla vive ahora en el motor (RULES.technicalWeight / marketWeight), y
    // duplicarla aqui era una de las formas en que dos pantallas mostraban
    // cifras distintas para la misma parcela.
    const suggestedCommunalValue=Number(tpl?.valorFinal||0);
    const urgencyPct=0.07;
    const urgencyValue=suggestedCommunalValue?Math.round(suggestedCommunalValue*(1-urgencyPct)):0;
    return {
      area, price, publishedM2, tpl, tplM2, market, marketM2:Number(market?.medianM2Ajustado||market?.medianM2||0),
      technicalValue, observedCommunalValue, suggestedCommunalValue, urgencyValue, urgencyPct,
      ...classification,
      hasValidatedMarket:Boolean(observedCommunalValue>0)
    };
  }

  const TPLMarketIntelligence=Object.freeze({ analyze, analyzeAsync, commercialTier, areaOf, priceOf, qualitySignals, REFERENCE_POINTS });
  if (typeof window !== 'undefined') window.TPLMarketIntelligence = TPLMarketIntelligence;
