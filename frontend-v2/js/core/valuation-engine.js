...
(function(global){
  ...
  function directAdjustments(input={}){
    ...
    // Calibrado el 2026-09-03 contra el catastro de mercado. Sobre 458 avisos
    // comparables de 3.000 a 12.000 m2, la mediana de $/m2 del suelo es:
    //
    //   sin cerco, sin porton   $6.000   n=286   (base)
    //   sin cerco, CON porton   $7.800   n= 57   +30%
    //   CON cerco, sin porton   $8.000   n= 79   +33%
    //   CON cerco, CON porton   $9.004   n= 36   +50%
    //
    // Los dos tienen efecto propio y se acumulan sin llegar a sumarse. Las
    // tarifas anteriores (+15% y +5%, con el grupo topado en +20%) se quedaban
    // muy cortas. Se sube a ~70% del efecto observado: el 30% restante se
    // atribuye a que una parcela cercada y con porton suele estar ademas mejor
    // mantenida y mas cerca de servicios, cosas que el motor ya puntua aparte.
    const fencing=normalize(input.fencing);
    if(/completamente|cerrado completo/.test(fencing))add('fence','Cierre perimetral completo',0.22,'+22%');
    else if(/parcialmente|deslindes|cerrado parcial/.test(fencing))add('fence','Cierre perimetral parcial',0.08,'+8%');
    ...
    if(/automatico|acceso controlado/.test(gate))add('gate','Portón y acceso controlado',0.18,'+18%');
    else if(/instalado|con porton/.test(gate))add('gate','Portón instalado',0.12,'+12%');
  ...
  function groupedAdjustmentFactor(adjustments=[]){
    ...
    // Tope subido de +20% a +35%: el mercado paga +50% por cerco y porton
    // juntos (n=36), asi que topar en +20% recortaba mas de la mitad del
    // efecto observado. +35% deja fuera la parte que probablemente ya cobran
    // otros ajustes.
    const readiness=1+clamp((byKey.fence||0)+(byKey.gate||0),-0.15,0.35);
