(() => {
  ...
  // Escapa antes de interpolar en innerHTML. Los nombres de parcelas y casas
  // provienen del catalogo y de localStorage; sin escapar, cualquiera de esas
  // fuentes podria inyectar HTML en paginas publicas.
  ...
  // findParcelFromPage() solo mira el catálogo estático (window.parcelas) y
  // localStorage. Una parcela publicada por CRM/portal y que aún no pasó por
  // sincronizar-catalogo-publico.mjs no está en ninguno de los dos, así que
  // esta función devolvía null y la ficha se quedaba SIN título propio, SIN
  // meta description, SIN og:image y SIN schema.org RealEstateListing —
  // invisible para buscadores aunque la ficha misma sí cargue los datos
  // (js/parcela.js ya resuelve esto mismo vía TPLDataService, ver
  // hydrateRemoteCatalog() ahí). Si el catálogo estático no tiene la parcela,
  // se intenta el mismo catálogo remoto que usa la ficha, y se repinta cuando
  // llega.
  function fetchParcelFromRemote(id) {
    if (!id || !window.TPLDataService?.getPublishedPropertyById) return Promise.resolve(null);
    return window.TPLDataService.getPublishedPropertyById(id)
      .then((row) => (row && window.TPLPropertyView?.normalizarPropiedad
        ? window.TPLPropertyView.normalizarPropiedad(row, { catalogoLocal: getParcelas() })
        : row))
      .catch(() => null);
  }

  function paintParcelInterestHero(p) {
    ...
    hero.className = "tpl-seo-banner";
    hero.innerHTML = `
      <div class="tpl-seo-banner__inner">
        <div class="tpl-seo-banner__copy">
          <strong>${esc(p.nombre || 'Tu Parcela')}</strong>
          <span>${chips.map(esc).join(' · ')}</span>
        </div>
        <div class="tpl-seo-banner__actions">
          <button type="button" data-tpl-scroll-detail class="tpl-seo-banner__btn">Ver detalles</button>
          <button type="button" data-tpl-prefab class="tpl-seo-banner__btn tpl-seo-banner__btn--primary">Cotizar prefabricada</button>
          <button type="button" data-tpl-own-house class="tpl-seo-banner__btn tpl-seo-banner__btn--outline">Diseño propio</button>
        </div>
      </div>
    `;
    ...
    const img = p.foto || p.imagen || (p.imagenes && p.imagenes.length ? p.imagenes[0] : null);
    const absImage = img ? `${BASE}/${img.replace(/^\//, "")}` : undefined;
    const pageUrl = `${BASE}/parcela.html?id=${encodeURIComponent(p.id)}`;
    const priceNumber = Number(String(p.precio || "").replace(/[^0-9]/g, "")) || undefined;
    window.TPLSEO.apply({
      ...
      schema: {
        ...
        "name": p.nombre || "Parcela",
        "url": pageUrl,
        ...(absImage ? { "image": absImage } : {}),
        "address": {
          "@type": "PostalAddress",
          "addressLocality": p.comuna || undefined,
          "addressRegion": p.region || undefined,
          "addressCountry": "CL"
        },
        ...(priceNumber ? { "offers": { "@type": "Offer", "price": priceNumber, "priceCurrency": "CLP", "availability": "https://schema.org/InStock", "url": pageUrl } } : {})
      }
    });
  }
  ...
  function renderParcelInterestHero() {
    ...
    if (p) { paintParcelInterestHero(p); return; }

    // No estaba en el catálogo estático ni en localStorage: puede ser una
    // parcela publicada dinámicamente y aún no sincronizada. Se intenta el
    // catálogo remoto y, si aparece, se pinta igual (más tarde, no bloquea
    // el resto de la página).
    const id = new URLSearchParams(location.search).get("id");
    fetchParcelFromRemote(id).then((remoto) => {
      if (remoto) paintParcelInterestHero(remoto);
    });
  }
  ...
  function decorateIndexHero() {
    ...
    hero.style.setProperty("--tpl-hero-photo-a", 'url("../image/publicar-parcela-fotografo-campo-v2.webp")');
  ...
  function activateCompactSelectedProject() {
    ...
    summary.innerHTML = `
      <div class="tpl-selected-combo-summary__media">
        <img src="${esc(parcelImage(parcel))}" alt="${esc(parcel.nombre || "Parcela")}">
        <img src="${esc(houseImage(house))}" alt="${esc(house.nombre || "Casa")}">
      </div>
      <div class="tpl-selected-combo-summary__body">
        <span>PROYECTO QUE ELEGISTE</span>
        <h3>${esc(parcel.nombre || "Parcela")} + ${esc(house.nombre || "Casa")}</h3>
        <p>${Number(parcel.tamano || 0).toLocaleString("es-CL")} m² de terreno · ${Number(house.metros || house.m2 || 0).toLocaleString("es-CL")} m² de casa · ${esc(house.habitaciones || house.dormitorios || "—")} habitaciones</p>
