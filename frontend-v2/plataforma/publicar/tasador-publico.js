...
let mode='rapida',assetType='parcela',lastResult=null,lastInput=null,catalog=null,tasadorContext=null,locationMap=null,locationMarker=null,valuationEngine=null;
...
function setAssetType(next){
 ...
 const isHouseOnly = ['casa','casa_depto'].includes(assetType);
 document.body.classList.toggle('is-house-only', isHouseOnly);
 
 const withHouse = ['parcela_casa','casa','casa_depto'].includes(assetType);
 const iv=$('#incluyeVivienda');if(iv)iv.checked=withHouse;
 const fields=$('#houseFields'); if(fields) fields.hidden=!withHouse;
 const houseToggles=document.querySelectorAll('.house-only-field');
 houseToggles.forEach(el=>el.hidden=!withHouse);
 
 const area=$('#superficie');
 if(area){ area.required = !isHouseOnly; if(isHouseOnly && !Number(area.value)) area.value='1'; }
 
 const status=$('#status'); 
 if(status) {
   if (assetType==='parcela') status.textContent='Tasación de terreno seleccionada.';
   else if (assetType==='parcela_casa') status.textContent='Tasación integrada de terreno y vivienda seleccionada.';
   else if (assetType==='terreno_urbano') status.textContent='Tasación de terreno urbano seleccionada.';
   else status.textContent='Tasación exclusiva de vivienda/depto seleccionada; el terreno no se suma.';
 }
}
...
function setMode(next){
 ...
 if(next==='precisa')setTimeout(()=>{initLocationMap();refreshLocationMap()},80);
}
function parseMoney(v){return Number(String(v||'').replace(/[^0-9]/g,''))||0}
function integerKm(v){const n=Number(String(v??'').replace(/[^0-9-]/g,''));return Number.isFinite(n)?Math.max(0,Math.round(n)):null}
function kmToMeters(v){const km=integerKm(v);return km===null?null:km*1000}
function normalizeDistanceInputs(){['communeDistanceKm','routeDistanceKm','electricityPoleDistanceKm'].forEach(id=>{const el=$('#'+id);if(!el)return;const n=integerKm(el.value);if(n!==null)el.value=String(n);});}
...
function refreshLocationMap(){
 ...
 [0,60,150,300,600,1000].forEach(delay=>setTimeout(()=>{try{locationMap.invalidateSize({pan:false,animate:false});const lat=Number($('#lat')?.value),lng=Number($('#lng')?.value);if(validCoordinate(lat,lng)&&lat&&lng)locationMap.panTo([lat,lng],{animate:false});}catch(_e){}},delay));
}
function initLocationMap(){
 const el=$('#tasadorMap');if(!el||!window.L)return;
 if(locationMap){refreshLocationMap();return;}
 locationMap=L.map(el,{scrollWheelZoom:true,zoomControl:true,doubleClickZoom:true,touchZoom:true,preferCanvas:true}).setView([-36.82,-73.05],7);
 const tiles=L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'&copy; OpenStreetMap',crossOrigin:true});
 tiles.addTo(locationMap);
 tiles.on('tileerror',()=>{const status=$('#locationStatus');if(status&&!status.classList.contains('is-ready'))status.innerHTML='<span class="location-dot"></span><div><strong>Mapa temporalmente incompleto</strong><small>Puedes ingresar coordenadas o pegar un enlace de Google Maps mientras se recargan los mosaicos.</small></div>';});
 locationMap.on('click',e=>setLocation(e.latlng.lat,e.latlng.lng,'mapa'));
 if(window.ResizeObserver){new ResizeObserver(()=>refreshLocationMap()).observe(el);if(window.frameElement)new ResizeObserver(()=>refreshLocationMap()).observe(window.frameElement);}
 window.addEventListener('resize',refreshLocationMap);document.addEventListener('visibilitychange',()=>{if(!document.hidden)refreshLocationMap()});
 refreshLocationMap();
...
function setLocationMethod(method){
 ...
 if(method==='mapa')setTimeout(()=>{initLocationMap();refreshLocationMap()},50);
}
...
function inputs(){
 ...
 const base={tipoActivo:assetType,soloVivienda:assetType==='casa',propiedadId:linkedPropertyId||null,propiedadCodigo:linkedPropertyCode||null,origen:launchParams.get('embed')==='crm'?'crm_tasador':'tasador_publico',area:assetType==='casa'?1:Number($('#superficie').value||0),communeDistanceKm:integerKm($('#communeDistanceKm')?.value),communeDistanceM:kmToMeters($('#communeDistanceKm')?.value),region:$('#region').value,comuna:$('#comuna').value,rol:$('#rol').value,electricity:$('#electricity').value,electricityPoleDistanceKm:integerKm($('#electricityPoleDistanceKm')?.value),electricityPoleDistanceM:kmToMeters($('#electricityPoleDistanceKm')?.value),topography:$('#topography').value,water:$('#water').value,tourismLevel:$('#tourismLevel').value,mode};
 if(mode==='precisa')Object.assign(base,{access:$('#access').value,routeDistanceKm:integerKm($('#routeDistanceKm')?.value),routeDistanceM:kmToMeters($('#routeDistanceKm')?.value),soil:$('#soil').value,exposure:$('#exposure').value,view:$('#view').value,vegetation:$('#vegetation').value,fencing:$('#fencing').value,gate:$('#gate').value,condominium:$('#condominium').value,asking:parseMoney($('#asking').value),nature,lat:Number($('#lat').value),lng:Number($('#lng').value)});
...
function territorialContextFor(x){
 ...
 try{
  ...
  if(Number.isFinite(x.lat)&&Number.isFinite(x.lng)&&x.lat&&x.lng){
   ...
   if(city?.centroid&&valuationEngine?.TPLLandEngine?.haversineKm) majorCityDistanceKm=valuationEngine.TPLLandEngine.haversineKm(point,city.centroid);
   if(communeCenter&&valuationEngine?.TPLLandEngine?.haversineKm) communeDistanceKm=valuationEngine.TPLLandEngine.haversineKm(point,communeCenter);
  }else if(communeCenter&&city?.centroid&&valuationEngine?.TPLLandEngine?.haversineKm){
   majorCityDistanceKm=valuationEngine.TPLLandEngine.haversineKm(communeCenter,city.centroid);
  }
...
async function calculate(ev){
 ...
 try{
  ...
  x.territorialIndex=valuationEngine?.TPLLandEngine?.calculateTerritorialIndex?.(x.nearbyContext||{}, {majorCityDistanceKm:x.majorCityDistanceKm,distanceKm:x.majorCityDistanceKm,tourism:x.tourism})||null;
  x.propertyIndex=valuationEngine?.TPLLandEngine?.calculatePropertyIndex?.(x)||null;
  ...
  let res=x.incluyeVivienda?valuationEngine?.TPLHouseEngine?.calculate(x):valuationEngine?.TPLLandEngine?.calculate(x);
  if(res){
    // Los comparables observados son una validación externa; nunca modifican el Valor Técnico TPL.
    res={...res,observedComparables:x.observedComparables,observedBlend:{technicalWeight:1,observedWeight:0,independent:true,technicalValue:Number(res.valorFinal||0),observedMedian:Number(x.observedComparables?.mediana_total||0)}};
  }
  ...
  $('#result').hidden=false;const tplTasador=Number(displayResult.valorFinal||0);const tplComunal=Number(displayResult.valorPromedioReferencia||0);const baseComunal=Number(displayResult.valorComunal||0);const apuro=Number(displayResult.valorPorApuro||0);$('#tplTasador').textContent=money(tplTasador);$('#tplTasadorComunal').textContent=money(tplComunal);$('#baseComunal').textContent=baseComunal?money(baseComunal):'Sin muestra';$('#valorApuro').textContent=money(apuro);const ufEl=$('#valorFinalUf');if(ufEl){ufEl.textContent=displayResult.valorFinalUf?`Equivalente referencial: ${Number(displayResult.valorFinalUf).toLocaleString('es-CL',{maximumFractionDigits:1})} UF · UF ${money(displayResult.ufClpUsed)}`:'Equivalente UF no disponible';}
  let persisted=null;
  try{
  ...
  const b=displayResult.desglose||{valorTerreno:displayResult.valorFinal};const labels={valorTerreno:'Terreno',valorCasa:'Vivienda',valorFundacion:'Fundación',sumaObrasAdicionales:'Obras adicionales','bonificaciónDiferenciadora':'Característica diferenciadora'};
  $('#breakdown').innerHTML=Object.entries(labels).filter(([k])=>Number(b[k])).map(([k,l])=>`<div class="breakdown-row"><span>${l}</span><strong>${money(b[k])}</strong></div>`).join('')||`<div class="breakdown-row"><span>Terreno</span><strong>${money(displayResult.valorFinal)}</strong></div>`;
  ...
  if(mr){
    box.hidden=false;
    const communalValue=Number(mr.medianValue||0);
    const technicalValue=Number(displayResult.valorFinal||0);
    const delta=communalValue?Math.round(((technicalValue-communalValue)/communalValue)*100):null;
    box.innerHTML=`<small>Valor Comunal TPL · referencia estadística independiente</small><strong>${money(communalValue||mr.medianM2) }${communalValue?'':'/m²'}</strong><span>Mediana ${money(mr.medianM2)}/m² · rango central ${money(mr.p25M2)}–${money(mr.p75M2)}/m² · confianza ${String(mr.confidence||'referencial').replace('-',' ')}</span>`;
    if(formula){
      formula.className='tpl-value-formula is-technical';
      formula.innerHTML=`<strong>Valor Técnico TPL independiente</strong><span>El mercado comunal no modifica este valor. Solo se muestra para comparar el análisis particular con el comportamiento estadístico de propiedades similares.</span>${delta!==null?`<div class="valuation-weight"><b>TPL ${delta>=0?'+':''}${delta}% vs. comuna</b><b>${delta>=0?'Atributos técnicos sobre el promedio':'Limitaciones técnicas bajo el promedio'}</b></div>`:''}`;
    }
  }else{
    box.hidden=true;
    if(formula){
      formula.className='tpl-value-formula is-technical';
      formula.innerHTML=`<strong>Valor Técnico TPL independiente</strong><span>No existe una referencia comunal validada para este segmento. La tasación se obtuvo exclusivamente desde los antecedentes técnicos de la propiedad.</span>`;
    }
  }
  const observed=displayResult.observedComparables;
  if(observed?.cantidad){
    box.hidden=false;
    box.innerHTML += `<hr><small>Mercado observado comparable</small><strong>${observed.cantidad} avisos similares</strong><span>${x.incluyeVivienda?`Casas de ${Math.max(20,x.areaCasa-20)}–${x.areaCasa+20} m² · ${x.dormitorios||'sin filtro'} dormitorios · `:''}confianza ${observed.confianza}. Solo referencia informativa; no modifica el Valor Técnico TPL.</span>`;
  }
  const q=new URLSearchParams({region:x.region,comuna:x.comuna,superficie:x.soloVivienda?'':x.area,tasacion:String(Math.round(displayResult.valorFinal)),tipo:x.tipoActivo||'parcela'});$('#publishLink').href=`index.html?${q.toString()}`;
 }catch(e){
...
async function openPremium(){
 ...
 <section class="premium-value"><small>Valor TPL Recomendado</small><strong>${money(r.valorFinal)}</strong><span>Venta por Apuro ${money(r.valorPorApuro)} · Valor Promedio de Referencia ${money(r.valorPromedioReferencia)}</span></section>
 ...
 <section><h3>Antecedentes</h3><table>${row('Región',x.region)}${row('Comuna',x.comuna)}${row('Superficie',`${x.area.toLocaleString('es-CL')} m²`)}${row('Distancia a ciudad grande',`${Math.round(Number(x.majorCityDistanceKm??x.distanceKm))} km`)}${row('Distancia al centro comunal',Number.isFinite(Number(x.communeDistanceKm))?`${Math.round(Number(x.communeDistanceKm))} km`:'No informada')}${row('Rol',x.rol)}${row('Electricidad',x.electricity)}${/factibilidad|postación|postacion/i.test(x.electricity||'')?row('Distancia a postación',`${Math.round(Number(x.electricityPoleDistanceKm??((x.electricityPoleDistanceM||0)/1000)))} km`):''}${row('Agua',x.water)}${row('Topografía',x.topography)}${x.mode==='precisa'?row('Acceso',x.access)+row('Suelo',x.soil)+row('Exposición solar',x.exposure)+row('Vista',x.view)+row('Vegetación',x.vegetation)+row('Cierre',x.fencing)+row('Acceso controlado',x.gate)+row('Condominio',x.condominium)+row('Distancia a ruta',`${Math.round(Number(x.routeDistanceKm||0))} km`):''}${x.incluyeVivienda?row('Casa',`${x.areaCasa} m²`)+row('Material',x.materialCasa)+row('Dormitorios',x.dormitorios)+row('Baños',x.banos)+row('Estado',x.estadoCasa):''}</table></section>
...
const CRM_FIELD_DEFS={
 ...
 electricity:{id:'electricity',label:'Electricidad'},electricity_pole_distance:{id:'electricityPoleDistanceKm',label:'Distancia a red eléctrica'},
...
async function saveOnlyFromCrm(){
 if(!linkedPropertyId){$('#status').textContent='No se recibió la propiedad que se debe actualizar.';return;}
 const button=$('#crmSaveOnly');if(button)button.disabled=true;
 try{
  normalizeDistanceInputs();
  normalizeDistanceInputs();
 const x=inputs(),territory=territorialContextFor(x);
  x.distanceKm=territory.majorCityDistanceKm;x.majorCityDistanceKm=territory.majorCityDistanceKm;x.communeDistanceKm=territory.communeDistanceKm;
  x.communeDistanceKm=Number.isFinite(Number(territory.communeDistanceKm))?Math.round(Number(territory.communeDistanceKm)):x.communeDistanceKm;
  x.communeDistanceM=Number.isFinite(Number(x.communeDistanceKm))?Number(x.communeDistanceKm)*1000:null;
  x.routeDistanceKm=integerKm($('#routeDistanceKm')?.value);x.routeDistanceM=kmToMeters($('#routeDistanceKm')?.value);
  x.tourism=territory.tourism;x.nearestCity=territory.city?{name:territory.city.name,category:territory.city.category||''}:null;
  if(!window.TPLDataService?.saveCrmValuationProperty)throw new Error('No está disponible el servicio de guardado.');
  await window.TPLDataService.saveCrmValuationProperty(linkedPropertyId,x,{});
  $('#status').textContent='Datos guardados correctamente en la ficha de la propiedad.';
  if(window.parent!==window)window.parent.postMessage({type:'TPL_PROPIEDAD_DATOS_GUARDADOS',propiedad_id:linkedPropertyId,entrada:x},window.location.origin);
 }catch(error){console.error(error);$('#status').textContent='No fue posible guardar los datos: '+(error?.message||'error desconocido');}
...
async function applyLaunchPrefill(){
 ...
 set('superficie','superficie');set('asking','asking');set('lat','lat');set('lng','lng');set('rol','rol');set('electricity','electricity');const poleMeters=get('electricity_pole_distance_m'); const poleKm=get('electricity_pole_distance'); if($('#electricityPoleDistanceKm')) $('#electricityPoleDistanceKm').value=poleKm!==null?String(integerKm(poleKm)??''):(poleMeters!==null?String(Math.round(Number(poleMeters)/1000)):'');set('water','water');set('tourismLevel','tourism');set('access','access');set('topography','topography');set('soil','soil');
 const communeMeters=get('commune_distance_m'); const communeKm=get('commune_distance'); if($('#communeDistanceKm')) $('#communeDistanceKm').value=communeKm!==null?String(integerKm(communeKm)??''):(communeMeters!==null?String(Math.round(Number(communeMeters)/1000)):'');
 set('exposure','exposure');set('view','view');set('vegetation','vegetation');set('fencing','fencing');set('gate','gate');set('condominium','condominium');
 const routeMeters=get('route_distance_m'); const routeKm=get('route_distance'); if($('#routeDistanceKm')) $('#routeDistanceKm').value=routeKm!==null?String(integerKm(routeKm)??''):(routeMeters!==null?String(Math.round(Number(routeMeters)/1000)):'');
...
document.addEventListener('DOMContentLoaded',async()=>{
 valuationEngine = { TPLLandEngine: window.TPLLandEngine, TPLHouseEngine: window.TPLHouseEngine };
 ...
 ['communeDistanceKm','routeDistanceKm','electricityPoleDistanceKm'].forEach(id=>$('#'+id)?.addEventListener('change',normalizeDistanceInputs));normalizeDistanceInputs();
