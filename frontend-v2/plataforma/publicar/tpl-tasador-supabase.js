...
async function register(input,result,ctx){
 ...
 try{if(global.TPLDataService?.registerValuation)registration=await global.TPLDataService.registerValuation(payload.input,payload.result)}catch(error){console.warn('TPL Tasador: registro de tasación pendiente.',error)}
