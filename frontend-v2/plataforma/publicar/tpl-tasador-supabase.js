...
function enrich(input,result,ctx){
 const ufClp=num(ctx?.uf?.valor_clp);
 const recommended=num(result?.valorFinal ?? result?.recommended ?? result?.ideal ?? result?.market);
