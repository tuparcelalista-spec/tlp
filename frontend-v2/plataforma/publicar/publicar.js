...
function toggleHouseFields(){
 ...
 const isHouse=selectedType==='parcela_casa'||selectedType==='casa'||selectedType==='casa_depto';
...
function data(){
 return {
  ...
  terreno:{
   ...
   luz:valueOf('#luzDetalle'),distanciaPosteM:valueOf('#distanciaPosteM')===''?null:numberOf('#distanciaPosteM'),acceso:valueOf('#accesoDetalle'),distanciaRutaPrincipalKm:numberOf('#distanciaRutaPrincipalKm'),
   cierre:valueOf('#cierreDetalle'),porton:valueOf('#portonDetalle'),rioDirecto:checked('#rioDirecto'),esteroNatural:checked('#esteroNatural'),vertienteNatural:checked('#vertienteNatural'),orillaLago:checked('#orillaLago'),termasNaturales:checked('#termasNaturales')
  },
  casa:['parcela_casa','casa','casa_depto'].includes(radioValue('tipo'))?{
  ...
  servicios:{
   ...
   naturaleza:/bosque nativo|bosque mixto|pradera|frutales/i.test(valueOf('#vegetacion'))||checked('#rioDirecto')||checked('#esteroNatural')||checked('#vertienteNatural')||checked('#orillaLago')||checked('#termasNaturales')
  },
...
function fill(d){
 ...
 set('#superficie',d.superficie);set('#suelo',d.suelo);
 ...
 check('#rioDirecto',t.rioDirecto);check('#esteroNatural',t.esteroNatural);check('#vertienteNatural',t.vertienteNatural);check('#orillaLago',t.orillaLago);check('#termasNaturales',t.termasNaturales);
 const c=d.casa||{};
...
function territorialContext(property){
 ...
 const tourism=commune?.tour ? (/destino tur[ií]stico|internacional|lacustre|tur/i.test(cityCategory)?'nacional':'local') : '';
...
function prevalidateValuation(){
 ...
 if(!hasCoords)missing.push({message:'Marca la ubicación en el mapa para que GEOINT calcule las distancias automáticamente.',el:$('#googleMapsLink')});
...
function setPopupChoice(choice){
 ...
 const map={tpl:technical,market};
 ...
 const selected=map[choice]||technical;
...
function populateValuationPopup(){
 ...
 const technical=Number(valuation.valorFinal||0),market=Number(valuation.valorComunal||0),recommended=Number(valuation.valorPromedioReferencia||0);
 ...
 function applyPopupValuation(){
  if(!valuation)return;
  const selected=Number(valuation.selectedValue||valuation.valorFinal||0);
  if(!selected)return;
  valuation.valorFinal=Math.round(selected/10000)*10000;
  const m2Reference = window.TPLMarketIntelligence?.REFERENCE_POINTS?.REFERENCE_PRICE_PER_M2 || 1800;
  valuation.valorPorApuro = Math.round((valuation.valorFinal + (m2Reference * (Number(valuation.area) || 0))) / 2);
  if(valuation.selectedMethod==='tpl') valuation.valorPromedioReferencia=valuation.valorFinal;
  else valuation.valorPromedioReferencia=Math.max(Number(valuation.valorPromedioReferencia||0),valuation.valorFinal);
...
async function loadNearbyServices(){
 ...
 try{
  ...
  const context=valuation?.nearbyContext||await TPLLandEngine?.fetchNearbyContext?.(lat,lng);
...
async function performCalculate(){
 ...
 try{
  ...
  const landInput={
   ...
   nature:[features.nature?'Bosque nativo':'',property.terreno?.vegetacion||'',property.terreno?.vistaPrincipal||'',property.terreno?.rioDirecto?'Río dentro o acceso directo':'',property.terreno?.esteroNatural?'Estero dentro o acceso directo':'',property.terreno?.vertienteNatural?'Vertiente':'',property.terreno?.orillaLago?'Orilla lago':'',property.terreno?.termasNaturales?'Termas':''].filter(Boolean),
   routeDistanceKm:Number(property.terreno?.distanciaRutaPrincipalKm||0),
   asking:property.precio,
   tipo:['parcela_casa','casa','casa_depto'].includes(property.tipo)?'casa':'parcela',lat:Number(property.coords?.lat)||null,lng:Number(property.coords?.lng)||null,
  ...
  const TPLLandEngine = window.TPLLandEngine, TPLHouseEngine = window.TPLHouseEngine;
  landInput.nearbyContext=null;
  landInput.territorialIndex=TPLLandEngine?.calculateTerritorialIndex?.(
  ...
  landInput.propertyIndex=TPLLandEngine?.calculatePropertyIndex?.(landInput)||null;
  let result;
  if(['parcela_casa','casa','casa_depto'].includes(property.tipo) && TPLHouseEngine?.calculate){
   const c=property.casa||{};
   const currentYear=new Date().getFullYear();
   const houseYear=Number(c.anio||0);
   const remodelYear=Number(c.anioRemodelacion||0);
   result=TPLHouseEngine.calculate({
  ...
  }else if(TPLLandEngine?.calculate){
  ...
  const technical=Number(result.valorFinal||0);
  ...
  valuation={
   id:id(),createdAt:new Date().toISOString(),area,distance,technical,
   urgency,urgencyPct:0,valorFinal:technical,valorComunal:Number(result.valorComunal||0),valorPromedioReferencia:Number(result.valorPromedioReferencia||0),valorPorApuro:Number(result.valorPorApuro||0),immediateReference:Number(result.immediateReference||0),asking:property.precio,
...
async function loadAnalyzer() {
 if(window.analyzeProperty) return window.analyzeProperty;
 try {
  const mod = await import('../../js/core/property-analyzer.js');
  window.analyzeProperty = mod.analyzeProperty;
  return mod.analyzeProperty;
 } catch (e) {
  console.warn(e);
 }
}
...
function buildValuationVirtues(property){
 if(window.analyzeProperty) {
  const result = window.analyzeProperty(property, null, null);
  return result.diagnosis.included.slice(0,6);
 }
 const out=[];
...
async function renderValuation(){
 await loadAnalyzer();
 $('#valuationMain').textContent=`Valor TPL Recomendado: ${money(valuation.valorFinal)}`;if(valuation.valorFinalUf&&valuation.ufClpUsed)$('#valuationMain').title=`${Number(valuation.valorFinalUf).toLocaleString('es-CL',{maximumFractionDigits:1})} UF · UF usada ${money(valuation.ufClpUsed)}`;
 $('#quickValue').textContent=money(valuation.valorPorApuro);$('#marketValue').textContent=money(valuation.valorFinal);$('#patientValue').textContent=money(valuation.valorPromedioReferencia);
...
function reportHTML(){
 ...
 const row=(label,value)=>value!==undefined&&value!==null&&String(value).trim()!==''?`<tr><td>${esc(label)}</td><td>${esc(value)}</td></tr>`:'';
 const services=[
 ...
 <h2>${esc(d.titulo||`${d.tipo} en ${d.comuna||'Chile'}`)}</h2>
 <p>Documento orientativo preparado con los antecedentes declarados y el análisis territorial disponible.</p>
 <div class="valuation-range"><div><small>Venta por Apuro</small><strong>${money(v.valorPorApuro)}</strong></div><div><small>Valor TPL Recomendado</small><strong>${money(v.valorFinal)}</strong></div><div><small>Valor Promedio de Referencia</small><strong>${money(v.valorPromedioReferencia)}</strong></div></div>
 ...
 ${row('Modelo TPL',money(v.valorFinal))}
...
function renderReview(){
 ...
 <section class="review-section"><h3>Propiedad</h3><div class="review-row"><span>Tipo</span><strong>${esc(d.tipo)}</strong></div><div class="review-row"><span>Ubicación</span><strong>${esc(d.comuna||'—')}, ${esc(d.region||'—')}</strong></div><div class="review-row"><span>Superficie</span><strong>${d.superficie.toLocaleString('es-CL')} m²</strong></div><div class="review-row"><span>Topografía</span><strong>${esc(t.topografia||d.suelo||'—')}</strong></div><div class="review-row"><span>Servicios</span><strong>${esc(serviceSummary)}</strong></div><div class="review-row"><span>Precio</span><strong>${money(d.precio)}</strong></div></section>
 ${c?`<section class="review-section"><h3>Casa</h3><div class="review-row"><span>Construcción</span><strong>${esc(c.superficie||'—')} m² · ${esc(c.material||'material por confirmar')}</strong></div><div class="review-row"><span>Programa</span><strong>${esc(c.habitaciones||'—')} dorm. · ${esc(c.banos||'—')} baños</strong></div><div class="review-row"><span>Estado</span><strong>${esc(c.estado||'—')} · ${esc(c.regularizacion||'regularización por confirmar')}</strong></div></section>`:''}
 <section class="review-section"><h3>Estrategia comercial</h3><div class="review-row"><span>Urgencia</span><strong>${esc(e.urgencia||'No indicada')}</strong></div><div class="review-row"><span>Plazo</span><strong>${esc(e.plazoVenta||'No indicado')}</strong></div><div class="review-row"><span>Negociación</span><strong>${esc(e.negociacionPrecio||'No indicada')}</strong></div><div class="review-row"><span>Visitas</span><strong>${esc(e.disponibilidadVisitas||'No indicada')}</strong></div></section>
 <section class="review-section"><h3>Diagnóstico y acuerdos</h3><div class="review-row"><span>Mejoras necesarias</span><strong>${esc(d.diagnostico.necesidades.map(x=>x.tipo.replaceAll('_',' ')).join(', ')||'Ninguna')}</strong></div><div class="review-row"><span>¿Acepta evaluar mejoras?</span><strong>${d.diagnostico.aceptaEvaluarMejoras==='si'?'Sí':d.diagnostico.aceptaEvaluarMejoras==='no'?'No por ahora':'Sin responder'}</strong></div></section>
 ...
 <section class="review-section"><h3>Responsable</h3><div class="review-row"><span>Nombre</span><strong>${esc(d.contacto.nombre||'—')}</strong></div><div class="review-row"><span>Correo</span><strong>${esc(d.contacto.email||'—')}</strong></div></section>`;
}
function showPublishSuccess(result,d,onboarding=null){
 ...
 if(mailStatus){
  mailStatus.innerHTML=onboarding?.ok
   ? `Confirmación y acceso enviados a <b>${esc(d?.contacto?.email||'tu correo')}</b>. Tu Plan Gratis quedó activado.`
   : `La publicación quedó registrada. El acceso y los correos están pendientes de reintento automático para <b>${esc(d?.contacto?.email||'tu correo')}</b>.`;
 }
 if(agendaLink){agendaLink.hidden=!onboarding?.ok;agendaLink.href=onboarding?.agenda_url||'../tpl-business-v2/'}
 if(!dialog.open)dialog.showModal();
}

async function checkPublishLimit() {
  if (!window.TPLDataService?.getSession) return true;
  try {
    const session = await window.TPLDataService.getSession();
    if (!session?.user) return true; // If not logged in, backend will handle creation or they just submit as lead
    
    // Si hay sesión, traemos el estado del CRM que ya resume plan y propiedades
    if (window.TPLDataService.getCrmSnapshot) {
       const snap = await window.TPLDataService.getCrmSnapshot();
       const plan = snap?.actor?.plan || 'gratis';
       const count = snap?.resumen?.propiedades || 0;
    ...
    console.warn('No se pudo verificar límite:', err);
    return true;
  }
}
...
async function submit(e){
 ...
 if (!canPublish) {
   if(btn){btn.disabled=false;btn.textContent='Publicar mi propiedad'}
   const pw = $('#paywallDialog');
   if (pw && !pw.open) pw.showModal();
   return;
 }
 ...
 const d={
  ...
  modelo_negocio: 'corretaje_2_pct' // Por defecto se inscribe al plan de corretaje digital a éxito
 };
...
const closePaywallBtn=$('#closePaywallBtn');if(closePaywallBtn)closePaywallBtn.onclick=()=>$('#paywallDialog')?.close();
const cancelPaywallBtn=$('#cancelPaywallBtn');if(cancelPaywallBtn)cancelPaywallBtn.onclick=()=>$('#paywallDialog')?.close();
