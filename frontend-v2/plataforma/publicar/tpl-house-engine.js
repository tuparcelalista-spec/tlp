...
(function(global){
  ...
  const RULES = Object.freeze({
    ...
    materials: Object.freeze({
      // Gama ligera: mediana de los 11 modelos prefabricados basicos del
      // catalogo TPL (casas.js), precio cotizado real, rango $109.524-$139.583.
      // Se deprecia mas rapido y su piso es mas bajo porque es tabiqueria
      // liviana montada sobre poyos, no obra.
      ligera: { baseM2: 125000, depAnnualM2: 6000, minM2: 45000, label: 'Construcción ligera / prefabricada' },
    ...
    additionalWorks: Object.freeze({
      ...
      // Una cabaña de visitas es construccion liviana: se valoriza con la misma
      // tarifa que la gama ligera del catalogo de prefabricadas ($125.000/m²),
      // que es literalmente lo que se levanta en esos casos.
      cabana_visitas: { base: 125000, unit: 'm2', depType: 'linear_10k', label: 'Cabaña / casa de visitas' }
    }),
  ...
  function calculate(input = {}) {
    ...
    let landResult = null;
    if (soloVivienda) {
      landResult = { valorFinal: 0, valorComunal: 0, area: 0, source: 'sin_terreno' };
      ...
      landResult = { valorFinal: landVal, valorComunal: landVal, area: landInput.area };
    }
    if (landResult?.error && !soloVivienda) return landResult;
    const valorTerreno = soloVivienda ? 0 : num(landResult.ideal || landResult.reference || landResult.valorFinal || 0);
    ...
    // 5.5. Castigo por falta de Recepción Municipal (-35% al valor de mejoras construidas)
    const recepcion = norm(input.recepcionFinal || input.recepcion || input.reception || 'sin_recepcion');
    const sinRecepcion = recepcion.includes('sin') || recepcion.includes('no');
    const factorRecepcion = (incluyeVivienda && sinRecepcion) ? 0.65 : 1.00;

    const valorMejoras = (valorCasa + valorFundacion + sumaObrasAdicionales) * factorRecepcion;
    const subtotalPropiedad = valorTerreno + valorMejoras;
    const valorComercialTotal = Math.round(subtotalPropiedad * factorDiferenciador);

    // 4 valores oficiales TPL V2.4
    const valorFinal = Math.round(valorComercialTotal / 10000) * 10000;
    
    // El valor comunal total suma el valor comunal del terreno más el valor de la construcción
    const valorComunalTerreno = num(landResult.valorComunal || 0);
    const valorComunal = soloVivienda ? valorFinal : Math.round(((valorComunalTerreno + valorMejoras) * factorDiferenciador) / 10000) * 10000;
    
    const valorPromedioReferencia = Math.round((valorFinal + valorComunal) / 2 / 10000) * 10000;
    const valorPorApuro = Math.round((valorFinal * RULES.quickFactor) / 10000) * 10000;
    ...
    return {
      ...
      valorPorApuro,
      valorComercialTotal: valorFinal,
      subtotalPropiedad,
      desglose: {
        valorTerreno,
        valorCasa,
