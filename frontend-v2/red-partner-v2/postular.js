...
const METHOD_TEMPLATE_RELATO = 'Cuando un cliente me contacta, primero visito el terreno para evaluar el alcance del trabajo. Luego preparo un presupuesto detallado y lo envío por WhatsApp o correo. Si el cliente acepta, coordinamos una fecha de inicio y solicito un anticipo. Durante la ejecución voy informando avances con fotos, y al terminar reviso el resultado junto al cliente antes de entregar.';
const METHOD_TEMPLATE_ETAPAS = [
  'Visita al terreno y evaluación del alcance del trabajo',
  'Presupuesto detallado enviado al cliente',
  'Acuerdo, anticipo y coordinación de fecha de inicio',
  'Ejecución del trabajo con avances informados',
  'Revisión final y entrega al cliente'
];

let methodClarityScore = null;
function setMethodScore(value) {
  methodClarityScore = value === null || value === undefined ? null : Number(value);
  const el = document.getElementById('method-score-value');
  if (el) el.textContent = methodClarityScore === null ? '—' : `${methodClarityScore}%`;
}

function setMethodSuggestions(message, type = 'info') {
  const box = document.getElementById('method-suggestions');
  if (!box) return;
  if (!message) { box.hidden = true; box.textContent = ''; return; }
  box.hidden = false;
  box.textContent = message;
  box.className = `method-suggestions is-${type}`;
}

const organizeButton = document.getElementById('organize-method');
organizeButton?.addEventListener('click', async () => {
  const textarea = document.getElementById('metodo_relato');
  const texto = String(textarea?.value || '').trim();
  if (texto.length < 20) {
    setMethodSuggestions('Escribe al menos un par de frases sobre tu forma de trabajo antes de ordenar en etapas.', 'error');
    return;
  }
  organizeButton.disabled = true;
  const originalLabel = organizeButton.textContent;
  organizeButton.textContent = 'Analizando…';
  setMethodSuggestions('Analizando tu explicación…', 'info');
  try {
    const response = await fetch(`${SUPABASE_URL}/functions/v1/gemini-metodo-trabajo`, {
      method: 'POST',
    ...
    if (!response.ok || !data?.ok) {
      const messages = {
        TEXTO_MUY_CORTO: 'Escribe un poco más sobre tu forma de trabajo.',
        DEMASIADAS_SOLICITUDES: 'Ya usaste varias veces esta función. Espera unos minutos e inténtalo de nuevo.',
        IA_NO_DISPONIBLE: 'El asistente no está disponible en este momento. Puedes ordenar las etapas manualmente.'
      };
      throw new Error(messages[data?.error] || 'No fue posible ordenar tu explicación en etapas. Puedes hacerlo manualmente.');
    }
    replaceRepeatableRows('service-stages-list', data.etapas, 'Ej: Visita y evaluación inicial');
    setMethodScore(data.claridad);
    setMethodSuggestions(`Detectamos ${data.etapas.length} etapas. Revísalas y ajústalas antes de guardar.`, 'success');
    updateProfileScore();
  } catch (error) {
    setMethodSuggestions(error.message, 'error');
  } finally {
    organizeButton.disabled = false;
    organizeButton.textContent = originalLabel;
  }
});

document.getElementById('load-method-template')?.addEventListener('click', () => {
  const textarea = document.getElementById('metodo_relato');
  if (textarea) textarea.value = METHOD_TEMPLATE_RELATO;
  replaceRepeatableRows('service-stages-list', METHOD_TEMPLATE_ETAPAS, 'Ej: Visita y evaluación inicial');
  setMethodScore(null);
  setMethodSuggestions('Cargamos una plantilla de ejemplo. Edítala para que refleje tu forma de trabajo real.', 'info');
  updateProfileScore();
});

document.getElementById('clear-method-template')?.addEventListener('click', () => {
  replaceRepeatableRows('service-stages-list', [], 'Ej: Visita y evaluación inicial');
  setMethodScore(null);
...
function buildPayload() {
  return {
    ...
    metodo_trabajo: {
      relato: document.getElementById('metodo_relato')?.value.trim() || '',
      score: methodClarityScore || 0,
      etapas: repeatableValues('service-stages-list')
    },
...
function fillSimplePayload(payload = {}) {
  ...
  const metodoRelato = document.getElementById('metodo_relato');
  if (metodoRelato && payload.metodo_trabajo?.relato) metodoRelato.value = payload.metodo_trabajo.relato;
  setMethodScore(payload.metodo_trabajo?.score || null);
