...
(async () => {
    ...
    // Palabras prohibidas para evitar "falsas comunas" (ej. Curarrehue camuflado de Villarrica)
    ...
    data.forEach(p => {
        ...
        // Filtro Anti-Trampas
        let isBlocked = blockList.some(word => titulo.includes(word) || localidad.includes(word));
        if (isBlocked) return;
        ...
        // REGLAS ESTRICTAS
        if (!precio || isNaN(precio) || precio < 8000000) return; // Ignorar falsos / muy baratos
        if (!m2 || isNaN(m2) || m2 < 4000 || m2 > 15000) return; // Ignorar Mega-campos o urbanos
    ...
    for (const c in grouped) {
        ...
        let cheapest = arr.slice(0, 10); // TOP 10 (o los que haya si son menos)
    ...
    // We will find the MARKET_REFERENCES block and replace it
    ...
    let md = '# Bases Comunales Definitivas (Top 10 Más Económicas + Filtro Anti-Trampas)\n\n';
    ...
    fs.writeFileSync('C:/Users/yo/.gemini/antigravity/brain/fb6b13b7-084c-4447-a45b-9c2a2e81035e/bases_definitivas.md', md, 'utf8');
    console.log('Bases actualizadas exitosamente con Top 10 y Filtros.');
})();
