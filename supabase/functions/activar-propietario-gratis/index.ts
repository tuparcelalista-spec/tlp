...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (existing?.estado === 'completado') {
      return jsonResponse({ ok: true, already_completed: true, estado: 'completado', agenda_url: `${siteUrl}/plataforma/tpl-business-v2/` });
    }
    ...
    const redirectTo = `${siteUrl}/plataforma/tpl-business-v2/`;
    ...
    const agendaUrl = `${siteUrl}/plataforma/tpl-business-v2/`;
      ...
      await supabase.from('tpl_notificaciones_actor').insert([
        {
          ...
          accion_url: '/plataforma/tpl-business-v2/',
