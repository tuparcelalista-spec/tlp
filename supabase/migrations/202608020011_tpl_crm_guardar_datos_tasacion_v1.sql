...
begin
  ...
  if not found then
    raise exception 'Propiedad no encontrada' using errcode = 'P0002';
  end if;
  ...
  v_metadata := coalesce(v_row.metadata, '{}'::jsonb) || jsonb_build_object(
    ...
    'tasacion_resultado_resumen', jsonb_strip_nulls(jsonb_build_object(
      'ideal', p_resultado->'ideal',
      'quick', p_resultado->'quick',
      'patient', p_resultado->'patient',
  ...
  set
    ...
    distancia_ruta_principal_km = coalesce(nullif(p_entrada->>'routeDistanceKm','')::numeric, distancia_ruta_principal_km),
    casa_datos = v_house,
    metadata = v_metadata,
    updated_at = now()
  ...
  return jsonb_build_object(
    'ok', true,
    'propiedad_id', v_row.id,
    'updated_at', v_row.updated_at,
    'modo', coalesce(p_entrada->>'mode','rapida'),
