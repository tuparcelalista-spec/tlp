...
const cutoffRegex = /\\n(?:Podr[íi]an interesarte|Propiedades similares|Otras propiedades|Sugerencias|Propiedades destacadas)(?:\\n|$)/i;
...
if (clpMatches.length > 0) {const explicitClpRegex = /(?:Precio|Valor)[^$]{0,50}\\$\\s*([0-9]{1,3}(?:\\.[0-9]{3}){1,3})/i;const explicitMatch = texto_seguro.match(explicitClpRegex);if (explicitMatch) {clp = parseInt(explicitMatch[1].replace(/\\./g, ''));}else {const sortedCLP = clpMatches.map(m => parseInt(m[1].replace(/\\./g, ''))).sort((a, b) => b - a);clp = sortedCLP[0];}}
const ufRegex = /UF\\s*([0-9]{1,3}(?:\\.[0-9]{3})*(?:,[0-9]+)?)/gi;
const ufMatches = [...texto_seguro.matchAll(ufRegex)];
if (ufMatches.length > 0) {const explicitUfRegex = /(?:Precio|Valor)[\\s\\S]{0,50}UF\\s*([0-9]{1,3}(?:\\.[0-9]{3})*(?:,[0-9]+)?)/i;const explicitMatchUf = texto_seguro.match(explicitUfRegex);if (explicitMatchUf) {uf = parseFloat(explicitMatchUf[1].replace(/\\./g, '').replace(',', '.'));}else {const sortedUF = ufMatches.map(m => parseFloat(m[1].replace(/\\./g, '').replace(',', '.'))).sort((a, b) => b - a);uf = sortedUF[0];}}
...
if (supMatches.length > 0) {let maxS = 0;let explicitS = null;for(let m of supMatches) {let matchText = m[1] || m[2];let isHa = m[0].toLowerCase().includes('ha') || m[0].toLowerCase().includes('hect');let s;if (matchText.includes('.') || matchText.includes(',')) {let normalized = matchText.replace(',', '.');if (!isHa && /\\.[0-9]{3}$/.test(normalized)) {s = parseFloat(normalized.replace('.', ''));}else {s = parseFloat(normalized);}}else {s = parseFloat(matchText);}if (isHa) s = s * 10000;if (m[1]) {explicitS = s;break;}if (s > maxS) maxS = s;}sup = explicitS || maxS;}
