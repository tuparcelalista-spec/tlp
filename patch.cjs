...
const startMarker = '          <div class="v3-price-block">\r\n            <div class="v3-price" id="v3-price"></div>\r\n            <div class="v3-price-note">Valor publicado</div>\r\n          </div>';
const startMarkerFallback = '          <div class="v3-price-block">\n            <div class="v3-price" id="v3-price"></div>\n            <div class="v3-price-note">Valor publicado</div>\n          </div>';

const endMarker = '      <div style="max-width: 600px; margin: 0 auto; display: flex; flex-direction: column; gap: 16px;">';

let idxStart = content.indexOf(startMarker);
if (idxStart === -1) {
    idxStart = content.indexOf(startMarkerFallback);
    if (idxStart !== -1) {
        startMarker = startMarkerFallback;
    }
}
...
`;
    const newContent = content.substring(0, idxStart + (startMarker !== -1 ? startMarker.length : startMarkerFallback.length)) + newMid + content.substring(idxEnd);
