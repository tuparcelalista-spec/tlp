...
(async () => {
    ...
    for (const c in grouped) {
        ...
        let cheapest = arr.slice(0, 10);
        
        // Calculate median
        let mid = Math.floor(cheapest.length / 2);
        let median = cheapest.length % 2 !== 0 ? cheapest[mid].m2_price : (cheapest[mid - 1].m2_price + cheapest[mid].m2_price) / 2;
        
        // REGLA DEL 40% MEJORADA: Sacamos los que están 40% por encima de la mediana (salto extremo) 
        // y también los que están 40% por debajo (spam/engaños que se saltaron el filtro de palabras).
        let accepted = cheapest.filter(x => x.m2_price <= median * 1.40 && x.m2_price >= median * 0.60);
        
        if (accepted.length < 2) accepted = cheapest; // fallback
    ...
    let md = '# Bases Comunales Definitivas (Con Filtro Bidireccional del 40%)\n\n';
    ...
    fs.writeFileSync('C:/Users/yo/.gemini/antigravity/brain/fb6b13b7-084c-4447-a45b-9c2a2e81035e/bases_finales.md', md, 'utf8');
})();
